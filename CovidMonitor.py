...
def main(option):
	...
	if option.casefold() in table.options_container:
		...
		try:
			start = time.time()
			print('\n\nSending Request...\n')
			ref_req = 'https://www.worldometers.info/coronavirus/' + table.options_container.get(option.casefold())
			req = requests.get(
			...
			with open('Errors.txt', 'a') as f:
				f.write(dt_string_time + ' Searching For: ' + option.capitalize() + ' ' + str(err) + '\n\n')
	...
	def scrape_write(pointer):
		with open(pointer + filename, 'a') as f:
			...
			for container in locator.findAll('div', {'class':'content-inner'})[0:]:
				records = [records.text.strip().replace(',','') for records in container.findAll('div', {'class':'maincounter-number'})[0:]]

				total_cases = records[0].replace('N/A', '0')
				total_deaths = records[1].replace('N/A', '0')
				total_recoveries = records[2].replace('N/A', '0')
				active_cases = int(total_cases) - int(total_deaths) - int(total_recoveries)

				f.write(country.capitalize().replace('é', 'e').replace('ç', 'c') + ', ' + dt_string + ', ' + total_cases + ', ' + total_deaths + ', ' + total_recoveries + ', ' + str(active_cases) + '\n')
			f.close()
...
def createChart(locator, country):
	...
	for container in locator.findAll('div', {'class':'content-inner'})[0:]:
		records = [records.text.strip() for records in container.findAll('div', {'class':'maincounter-number'})[0:]]
		...
		labels = ["Cases: " + records[0].replace('N/A','0'), "Deaths: " + records[1].replace('N/A','0'), "Recoveries: " + records[2].replace('N/A','0'), "Active Cases: " + str(active_cases)]
		...
		plt.savefig(dt_string + '__' + country.capitalize() + '.png', bbox_inches='tight')
...
def transferPhoto(src, country):
	destination = src + "Covid Pie Charts/" + country.capitalize() + "/" + year + "/" + month + "/"
	...
	try:
		if os.path.exists(destination + fileformat):
			print('\n\nImage saved and transferred successfully to: ' + destination + '\n\nas: ' + fileformat + '\n')
...
def getInfo(locator):
	for container in locator.findAll('div', {'class':'content-inner'})[0:]:
		records = [records.text for records in container.findAll('div', {'class':'maincounter-number'})[0:]]
		print('{' + dt_string_time + '} | Cases: ' + records[0].strip().replace('N/A','0') + ' | Deaths: ' + records[1].strip().replace('N/A','0') + ' | Recoveries: ' + records[2].strip().replace('N/A','0'))
...
if __name__ == '__main__':
	...
	for country in table.options_container:
		...
		dt_string, dt_string_time = now.strftime("%B %d-%Y"), now.strftime("%B %d-%Y | %H:%M:%S")
		main(country)
